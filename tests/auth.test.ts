...
const identify = (identifier: string, password: string, schoolId?: number) =>
  request(app).post('/auth/identify').send({ identifier, password, schoolId });
...
describe('POST /auth/identify — connexion, sans sous-domaine ni en-tête', () => {
  ...
  it('refuse un compte archivé, avec le même statut qu’un compte inconnu', async () => {
    await createUser({
      schoolId: school.id,
    ...
    const archived = await identify('archive@a.test', TEST_PASSWORD);
    const unknown = await identify('inconnu@a.test', TEST_PASSWORD);
  ...
  it('renvoie la liste des écoles quand le même identifiant et mot de passe valent dans deux écoles', async () => {
    ...
    const ids = res.body.schools.map((s: { id: number }) => s.id).sort((a: number, b: number) => a - b);
    expect(ids).toEqual([school.id, other.id].sort((a, b) => a - b));
  });
  ...
  it('termine la connexion sur l’école choisie parmi une liste ambiguë', async () => {
    const other = await createSchool('ecole-b');
    await createUser({ schoolId: other.id, email: 'parent@a.test', role: 'parent' });

    const ambiguous = await identify('parent@a.test', TEST_PASSWORD);
    expect(ambiguous.body.status).toBe('ambiguous');

    const res = await identify('parent@a.test', TEST_PASSWORD, other.id);
    expect(res.status).toBe(200);
    expect(res.body.status).toBe('ok');
    expect(res.body.school.id).toBe(other.id);
  });

  it('refuse une école qui ne fait pas partie des comptes vérifiés', async () => {
    const other = await createSchool('ecole-b');
    const res = await identify('parent@a.test', TEST_PASSWORD, other.id);
    expect(res.status).toBe(401);
  });
...
describe('cycle refresh / logout', () => {
  const login = () => identify('parent@a.test', TEST_PASSWORD);
...
describe('réinitialisation de mot de passe', () => {
  ...
  it('envoie un lien par compte quand le même email existe dans plusieurs écoles', async () => {
    const other = await createSchool('ecole-b');
    await createUser({ schoolId: other.id, email: 'parent@a.test', role: 'parent' });
  ...
  it('change le mot de passe et invalide les sessions en cours', async () => {
    const session = await identify('parent@a.test', TEST_PASSWORD);
    await forgot('parent@a.test');
    ...
    expect((await identify('parent@a.test', TEST_PASSWORD)).status).toBe(401);
    expect((await identify('parent@a.test', 'nouveaumotdepasse')).status).toBe(200);
